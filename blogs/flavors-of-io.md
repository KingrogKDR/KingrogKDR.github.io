...
"slug": "flavors-of-io",
"date": "2026-08-09",
"tag": ["System Internals", "IO", "Memory", "Storage"],
"excerpt": "A deep dive into different flavors of IO and how each data is actually read/written to disk.",
"series": "disk-io",
"pinned": true
}
...
> [!note] When performing a write operation that is backed by the kernel or by a library buffer, it is important to make sure that the data actually reaches stable storage, i.e disk. The errors will only appear when the data is finally stored in disk, which can be while _fsyncing_ or closing the file. So, it becomes really important to observe the error code to know where it actually failed.
